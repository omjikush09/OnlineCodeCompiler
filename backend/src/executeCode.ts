...
const executePyCode = (filepath: string) => {
	...
	return new Promise((resolve, reject) => {
		exec(
			` docker run  --rm -v "${dirCode}:/usr/src/myapp -w /usr/src/myapp" python:latest python ${id}.py`,
			(error, stdout, stderr) => {
				console.log(error);
...
const executeCppCode = async (filepath: string) => {
	// const execute=()
	...
	return new Promise((resolve, reject) => {
		...
		exec(
			`docker run --rm -v "${dirCode}":/usr/src/myapp -w /usr/src/myapp gcc:latest g++ -o ${id} ${id}.cpp`,
			(error, stdout, stderr) => {
				console.log(error);
				if (error) {
				...
				// resolve(stdout);
				exec(
					`docker run --rm -v ${dirStr}:/usr/src/myapp -w /usr/src/myapp gcc:latest ./${id}`,
